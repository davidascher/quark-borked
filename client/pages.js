...
Template.editablepagetitle.editing_title = function() {
  ...
  return page && page.editing;
}

Template.editablepagetitle.events({
  'click span.pagetitle': function(evt) {
    var page = Pages.findOne({name: Session.get("page_name")});
    console.log('toggling', page, page.editing)
    if (page) {
      pagePages.update(page._id, {$set: {editing: !page.editing}})
    }
  }
})
...
Template.heart.events({
  'click i.heart': function(evt, template) {
  ...
  if (contents.length == 0) {
    ...
    if (numsubparas > 1) {
      ...
      for (var i = 0; i < subparas.length; i++) {
        console.log(pageName, index + (i / numsubparas), subparas[i]);
        Paras.insert({
          page: pageName,
